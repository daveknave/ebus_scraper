...
def load_article_data(soup, identifier_string, memory_arr):
   for pagelink in soup.select(identifier_string):
      ...
      for s in article_soup.select('script'):
         s.extract()
      article_data['text'] = str(article_soup.select_one('.prose'))
...
def scrape():
   result = requests.get('https://www.sustainable-bus.com/category/infrastructure/')
